...
void loop() {
  ...
  Serial.println("loop");
  Serial.println(lightSensorIsWhite(rightLightSensorPin, 0));
  keep_straight();
  
}

void align_left() {
  while (lightSensorIsWhite(middleLightSensorPin, 5) == 0) {
...
void keep_straight() {
  ...
  if ((lightSensorIsWhite(leftLightSensorPin, 10) == 1) && (lightSensorIsWhite(middleLightSensorPin, 10) == 0)){
    align_left();
  }
  if ((lightSensorIsWhite(rightLightSensorPin, 10) == 1) && (lightSensorIsWhite(middleLightSensorPin, 10) == 0)) {
    align_right();
  }
